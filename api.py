...
class GameAPI:
    ...
    def fetch_all_games(self, limit=40):
        ...
        if not games:
            return []
        return self.deduplicate_games(games)[:limit]
    ...
    def fetch_cheapshark_free(self, limit=20):
        ...
        for deal in data[:limit]:
            ...
            games.append({
                ...
                "worth": float(deal.get("normalPrice", 0) or 0),
                "worth": float(str(item.get("worth", "0")).replace("$", "").replace("N/A", "0") if item.get("worth") else 0),
                "worth": float(str(item.get("worth", "0")).replace("$", "").replace("N/A", "0").replace(" ", "") if item.get("worth") else 0),
                "price": "FREE",
                "period": "TBD",
                "image": img,
                "link": link,
