...
def create_game_card(parent, game, details_callback):
    ...
    card = ctk.CTkFrame(parent, fg_color=CARD_FG, height=160, corner_radius=16, border_width=2, border_color=CARD_BORDER)
    ...
    if img_url and img_url in _IMG_CACHE:
        ...
        def _worker():
            try:
                ...
                proxy = f"https://wsrv.nl/?url={encoded}&w=220&h=260"
                
                try:
                    resp = requests.get(proxy, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}, timeout=(3.0, 5.0))
                    resp.raise_for_status()
                except Exception:
                    resp = requests.get(img_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}, timeout=(3.0, 5.0))
                    resp.raise_for_status()
                ...
                def _apply():
                    try:
                        if not img_label.winfo_exists(): return
                        ctk_img = CTkImage(light_image=pil_img, size=pil_img.size)
                        
                        # Сохраняем в кэш и ограничиваем его 100 элементами
    ...
    ctk.CTkLabel(content, text=title_show, font=ctk.CTkFont(size=18, weight="bold"), text_color=THEMES["dark"]["text"]).place(x=0, y=0)

    platform = (game.get("platform") or "").strip()
    if platform:
        ctk.CTkLabel(content, text=platform, font=ctk.CTkFont(size=13, weight="bold"), text_color="white", fg_color="#4da3ff", padx=12, pady=4, corner_radius=6).place(x=0, y=30)

    if is_new:
        ctk.CTkLabel(content, text="Новая игра", font=ctk.CTkFont(size=12, weight="bold"), text_color="white", fg_color=NEW_BADGE_BG, padx=10, pady=4, corner_radius=6).place(x=140, y=30)

    price_raw = (game.get("price") or "FREE").strip()
    price_show = "Бесплатно" if price_raw.upper() == "FREE" else price_raw
    ctk.CTkLabel(content, text=price_show, font=ctk.CTkFont(size=24, weight="bold"), text_color="#00ff88" if price_raw.upper() == "FREE" else "#ffb020").place(x=0, y=62)
    ...
    if score:
        try: info_parts.append(f"Рейтинг {float(score):.1f}")
    ...
    if info_parts:
        ctk.CTkLabel(content, text="  |  ".join(info_parts), font=ctk.CTkFont(size=12, weight="bold"), text_color=THEMES["dark"]["text_secondary"]).place(x=0, y=98)
    ...
    if ending_soon:
        ...
        if isinstance(hrs, (int, float)):
            tail = f" ~{hrs:.1f}ч"
        ctk.CTkLabel(content, text="Скоро закончится" + tail, font=ctk.CTkFont(size=12, weight="bold"), text_color="white", fg_color=SOON_BADGE_BG, padx=10, pady=4, corner_radius=6).place(x=0, y=128)
    ...
    for btn in extra_hover_btns:
        try: btn.bind("<Enter>", lambda e: card_on_enter())
        except: pass
