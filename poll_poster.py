...
def send_vk_poll_post(question, options):
    ...
    vk_text = f"🔥 <b>Опрос дня!</b>\n\n🤔 <b>{question}</b>\n\n"
    for i, opt in enumerate(options):
        emoji = numbers[i] if i < len(numbers) else "👉"
        vk_text += f"{emoji} {opt}\n"
        
    vk_text += "\n👇 <i>Пишите цифру или свой развернутый ответ в комментарии! Посмотрим, кого здесь больше!</i>"
    
    # Отправляем через ваш уже настроенный модуль ВК (без картинки, только текст)
    try:
        send_vk_wall_post(vk_text, image_url="", fallback_image_url="")
...
def main():
    ...
    if not available_polls:
        ...
        # Можно автоматически сбросить статус всех опросов, раскомментировав строки ниже:
        # for p in polls: p["used"] = False
        # save_polls(polls)
        return
