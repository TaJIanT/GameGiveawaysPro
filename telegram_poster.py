...
# --- СЛОВАРИ ДЛЯ РАНДОМИЗАЦИИ ПОСТОВ ---
HEADERS = [
    "🔥 ЛУТАЕМ ХАЛЯВУ", 
    "🚨 СВЕЖИЙ ДРОП", 
    "⚡️ СРОЧНО НА АККАУНТ", 
...
def send_to_telegram(game):
    ...
    # Выбираем случайные элементы для поста
...
def main():
    if not TG_TOKEN or not TG_CHAT_ID:
        print("❌ ОШИБКА: Секреты TG_TOKEN или TG_CHAT_ID не найдены!")
        sys.exit(1)
    ...
    if new_freebies:
        for game in new_freebies:
            ...
            time.sleep(2) # Защита от спам-блока при массовой отправке
    else:
